...
void Ans()
{
    ...
    for(int i = 0; i < 5; ++i)
    {
        ...
        scanf("%c", &action);
        clstd();
        if(toupper(action) == 'A' || toupper(action) == 'C' || toupper(action) == 'D')
        {
            break;
        }
...
{   
    ...
    for(int i = 0; i < menu.total; i++)
    {
        ...
        {   
            if(strcmp(TempOrder[j].status, ORDER_PAID) != 0) continue;
    ...
    *revenue = 0;
    ...
    {   
        if(strcmp(TempOrder[i].status, ORDER_PAID) != 0) continue;
